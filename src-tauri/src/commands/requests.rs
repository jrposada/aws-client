...
pub async fn get_active_request<'r>(
    ...
    let active_request = app_state.active_request.lock().await.clone();
    info!("<<< get_active_request");
    return Ok(active_request);
}

#[tauri::command]
pub async fn get_open_requests<'r>(app_state: State<'r, AppState>) -> Result<Vec<Request>, String> {
    info!(">>> get_open_requests");
    let open_requests = app_state.open_requests.lock().await.clone();
    info!("<<< get_open_requests");
    return Ok(open_requests);
}

#[tauri::command]
pub async fn get_requests<'r>(app_state: State<'r, AppState>) -> Result<Vec<Request>, String> {
    info!(">>> get_requests");
    let requests = app_state.requests.lock().await.clone();
    info!("<<< get_requests");
    return Ok(requests);
}
...
pub async fn post_requests<'r>(
    ...
    app_state
        .add_request(request_type.parse::<RequestType>()?, title)
        .await?;
    info!("<<< post_requests {:?}", request_type);
    return Ok(());
}
...
pub async fn put_requests<'r>(
    ...
    let request = app_state.update_request(id, title, data).await?;
